...
Firefox Developer Edition is supported on Windows and Linux. Windows uses the shared Mozilla Firefox registry policy, while Linux archive installations receive a policy in their own `distribution` directory. See the [Firefox guide](/firefox) for detection, verification, and uninstall details.
